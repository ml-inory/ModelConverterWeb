...
def _convert_conv(node, graph, err):
    ...
    # else:
    #     layer = myf("DepthwiseConv", node_name, [input_name], [output_name],
    #                 kernel_h = kernel_shape[0],kernel_w = kernel_shape[1],
    #                 stride_h=strides[0], stride_w = strides[1], #group = groups,
    #                 pad_h = pads[0], pad_w = pads[1],
    #                 num_output=W.shape[0],  dilation = dilations[0], bias_term = bias_flag)
...
def _convert_Reshape(node,graph,err):
    ...
    if len(shape) == 2:
        ...
        return err.unsupported_op_configuration(node, "Reshape dimention number shall be 2 or 3 or 4")
...
def _convert_gemm(node,graph,err):
    ...
    if 'broadcast' in node.attrs.keys() and node.attrs["broadcast"] != 1 or node.attrs["transB"] != 1:
...
def _convert_gap(node, graph, err):
    node_name = node.name
    input_name = str(node.inputs[0])
    output_name = str(node.outputs[0])
    pool_type = P.Pooling.AVE
...
def _convert_softmax(node, graph, err):
    ...
    layer = myf("Softmax",name,[input_name],[output_name])
    # l_top_relu1 = L.ReLU(l_bottom, name=name, in_place=True)
...
_ONNX_NODE_REGISTRY = {
    ...
    "Gemm": _convert_gemm,
    "Upsample": _convert_upsample2,
    "Concat": _convert_concat,
    "ConvTranspose": _convert_conv_transpose,
    "Sigmoid": _convert_sigmoid,
    "Flatten": _convert_Flatten,
    "Clip": _convert_relu,
    'ReduceMean': _convert_gap,
    'GlobalAveragePool': _convert_gap,
